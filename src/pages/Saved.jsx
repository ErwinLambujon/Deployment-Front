...
const Saved = () => {
		...
		const getSavedProblemStatement = async () => {
			...
			try {
				...
				let response1 = await axios.get(
					"https://deployment-backend-u1v3.onrender.com/api/two_venn_ps/",
					{
				...
				let response = await axios.get(
					"https://deployment-backend-u1v3.onrender.com/api/three_venn_ps/",
					{
						headers: { Authorization: `Token ${token}` },
					}
				);
		...
		if (setting === 2) {
			try {
				let response = await axios.put(
					`https://deployment-backend-u1v3.onrender.com/api/two_venn_ps/${id}/`,
					{
						statement: statement,
					},
					{
			...
			try {
				let response = await axios.put(
					`https://deployment-backend-u1v3.onrender.com/api/three_venn_ps/${id}/`,
					{
						statement: statement,
					},
					{
	...
	const handleDelete = async (setting, id) => {
		if (setting === 2) {
			try {
				let token = localStorage.getItem("token");
				await axios.delete(`https://deployment-backend-u1v3.onrender.com/api/two_venn_ps/${id}/`, {
					headers: { Authorization: `Token ${token}` },
				});
			...
			try {
				let token = localStorage.getItem("token");
				await axios.delete(`https://deployment-backend-u1v3.onrender.com/api/three_venn_ps/${id}/`, {
					headers: { Authorization: `Token ${token}` },
				});
