...
const PendingCards = ({ roomId, onAccept }) => {
    ...
    useEffect(() => {
        const fetchPendingMembers = async () => {
            ...
            try {
                const response = await axios.get(`https://deployment-backend-u1v3.onrender.com/api/rooms/${roomId}/applicants/`, {
                    headers: {
                ...
                const applicantsWithUserData = await Promise.all(
                    pendingApplicants.map(async (applicant) => {
                        try {
                            const userResponse = await axios.get(`https://deployment-backend-u1v3.onrender.com/api/user/${applicant.user}`, {
                                headers: {
                                    Authorization: `Token ${token}`,
                                },
                            });
    ...
    const handleAction = async (requestId, action) => {
        ...
        try {
            await axios.post(
                `https://deployment-backend-u1v3.onrender.com/api/rooms/${roomId}/manage_request/`,
